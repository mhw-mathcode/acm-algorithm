...
struct MaxCostMatch {
    ...
    void getMatch(int x, int y) { // 获取匹配方案 (0代表无匹配)
        for (int i = 1; i <= x; ++i) {
...
struct MaxCostMatch_double {
    vector<int> ansl, ansr, pre;
    vector<double> lx, ly;
    vector<vector<double>> ver;
    int n;

    MaxCostMatch_double(int n) : n(n) {
        ver.resize(n + 1, vector<double>(n + 1));
        ansl.resize(n + 1, -1);
        ansr.resize(n + 1, -1);
        lx.resize(n + 1);
        ly.resize(n + 1, -1E18);

...
struct Line3
{
	...
	// 点到直线距离
	double point_line_dis(Point3 p) { return ((e - s) ^ (p - s)).len() / s.distance(e); }
	// 点到线段距离
...
struct Plane // 只有a点与法向量即可代表平面
{
	...
	int point_plane(Point3 p) { return sgn((p - a) * o); }
	...
	// 两平面夹角
	double plane_angle(Plane f) { return acos(o * f.o) / (o.len() * f.o.len()); }
	// 平面和直线的交点，返回值是交点个数
